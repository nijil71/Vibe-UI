    ...
    return (
        <div className="w-full max-w-7xl mx-auto py-16 md:py-24 px-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
                <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent-blue/10 border border-accent-blue/20 text-xs font-bold text-blue-400 uppercase tracking-widest mb-6">
                        Developer SDK
                    </div>
                    <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight">Deploy in seconds, not hours.</h2>
                    <p className="text-neutral-400 text-base md:text-lg mb-10 leading-relaxed max-w-xl">
                ...
                <div className="relative mt-8 lg:mt-0">
                    ...
                    <div className="relative bg-neutral-900/50 backdrop-blur-xl border border-white/10 rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl">
                        <div className="flex items-center justify-between px-4 md:px-6 py-4 bg-white/5 border-b border-white/5">
                            ...
                            <button
                                onClick={handleCopy}
                                className="p-2 rounded-lg hover:bg-white/5 text-neutral-400 transition-colors shrink-0 ml-2"
                            >
                                {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
                            </button>
                        </div>
                        <div className="p-4 md:p-8 overflow-x-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                            <pre className="text-[12px] md:text-sm font-mono leading-relaxed min-w-full inline-block">
                                <code className="text-neutral-300">
                                    {languages[activeLang].code.split('\n').map((line, i) => (
                                        <div key={i} className="table-row">
                                            <span className="table-cell pr-4 md:pr-6 text-neutral-600 select-none text-right w-6 md:w-8 italic">{i + 1}</span>
                        ...
                        <div className="px-4 md:px-6 py-4 bg-white/5 border-t border-white/5 flex items-center justify-between">
                            <div className="flex gap-2">
                                <div className="w-2 h-2 rounded-full bg-red-500/20" />
                                <div className="w-2 h-2 rounded-full bg-yellow-500/20" />
                                <div className="w-2 h-2 rounded-full bg-green-500/20" />
                            </div>
                            <span className="text-[9px] md:text-[10px] text-neutral-600 font-mono tracking-tighter">SDK VERSION: 4.2.0-STABLE</span>
                        </div>
                    </div>
                </div>
            </div>
